...
void HCSR04Agent::addSensor(uint trigger, uint echo, const char * name){
	pSensors[xSensorCount] = new  HCSR04Range(trigger, echo);
 ...
 */
 ...
 void HCSR04Agent::publishRange(uint sensorIndex){
	 ...
	 xMsgsRange[sensorIndex].range = (float) pSensors[sensorIndex]->getDistanceMeters();
	 ...
	 if (xMsgsRange[sensorIndex].range < 0.0){
		 //Error so don't publish
		 return;
	 }
