...
void LX16A::setLimitDeg(uint8_t servo, float minDeg, float maxDeg){
	if (minDeg >= maxDeg){
		return;
	}
	if ((minDeg >= 0.0) && (minDeg <= 360.0)){
		xMinDeg = minDeg;
	}
	if ((maxDeg >= 0.0) && (maxDeg <= 360.0)){
		xMaxDeg = maxDeg;
	}
}

void LX16A::setLimitRad(uint8_t servo, float minRad, float maxRad){
	float l = minRad * 180.0/M_PI;
	float h = maxRad * 180.0/M_PI;
	setLimitDeg(servo, l,  h);
}
